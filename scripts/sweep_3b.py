"""Unattended stage-3b sweep driver: trains the 6 non-X3D 3b_*.yaml configs
fresh (30 epochs, no resume), cheapest-first, then per config runs
predict -> report -> cost harness -> git commit/push. Same structure as
sweep_3a.py.

X3D-S/X3D-M are excluded here: they raise a hard RuntimeError at 112px
(their downsampling schedule expects 160/224px input) and are handled by
sweep_3b_x3d.py with the upsample wrapper in src/models.py.

    .venv\\Scripts\\python scripts/sweep_3b.py
"""
...
# Cheapest-first, by measured GPU forward latency (2026-08-18 smoke test)
# scaled against known stage-3a r2plus1d_18 epoch times.
CONFIGS = [
    "3b_r2dpool_f16_p4",
    "3b_r2dpool_f32_p2",
    "3b_r3d_f16_p4",
...
def main():
    LOGS.mkdir(exist_ok=True)
    sweep_log = LOGS / "sweep3b_progress.log"
    sweep_alert = LOGS / "sweep3b_ALERT.txt"

    append(sweep_log, f"{now_iso()} === FRESH stage-3b sweep started, "
                       f"{len(CONFIGS)} configs (x3d_s/x3d_m excluded -- "
                       f"incompatible with 112px input, see module docstring), "
    ...
    for name in CONFIGS:
        ...
        if not ok:
            ...
            status = git_commit_and_push(f"Stage 3b sweep HALTED at {name}: training failure (see logs)")
        ...
        try:
            ...
            status = git_commit_and_push(f"Stage 3b sweep HALTED at {name}: eval/cost failure (see logs)")
            append(sweep_log, f"{now_iso()} git: {status}\n")
            return

        status = git_commit_and_push(f"Stage 3b: complete fresh 30-epoch run for {name}")
    ...
    status = git_commit_and_push("Stage 3b sweep complete: 6 configs (r3d_18/mc3_18/r2d_18_pool), fresh 30-epoch runs")
